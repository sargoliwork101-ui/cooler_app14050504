...
endpoint زیر اضافه شده است:

```text
POST /factory-reset
```

payload شامل کلید `code` است. کد امنیتی این عملیات در firmware ثابت است و در رابط کاربری/مستندات کاربر نمایش داده نمی‌شود. با اجرای این endpoint، `LittleFS.format()` انجام می‌شود و همه سناریوها، زمان‌بندی‌ها، تنظیمات WiFi، ساعت ذخیره‌شده، آمار موتور، تنظیمات محافظت و اطلاعات NTP پاک می‌شوند. سپس برد بدون ذخیره دوباره زمان، ری‌استارت می‌شود و به مقادیر پیش‌فرض firmware برمی‌گردد.

## اصلاح آمار کارکرد موتور

برای جلوگیری از اضافه شدن عددهای پرت به «مجموع مدت کارکرد موتور»، در نسخه جدید چند محافظ اضافه شده است:

- فایل‌های آمار رله هنگام خواندن اعتبارسنجی می‌شوند و اگر `switchCount` یا `onSeconds` غیرواقعی باشد، رکورد رد می‌شود.
- تایمر ذخیره دوره‌ای آمار در زمان بوت مقداردهی می‌شود تا اختلاف از صفر `millis()` حساب نشود.
- مدت دوره جاری روشن بودن با `relayCurrentPeriodElapsedSeconds()` محاسبه می‌شود و اگر مقدار غیرمنطقی باشد، به مجموع اضافه نمی‌شود.
- اضافه شدن به `relayTotalOnSeconds` از طریق `addRelayOnSecondsSafely()` انجام می‌شود تا overflow یا جهش بزرگ رخ ندهد.

اگر قبلاً روی برد فایل آمار خراب با عدد خیلی بزرگ ذخیره شده باشد، بعد از آپلود نسخه جدید و ری‌استارت، رکوردهای غیرواقعی توسط firmware نادیده گرفته می‌شوند.
