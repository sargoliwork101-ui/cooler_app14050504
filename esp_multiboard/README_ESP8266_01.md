...
رمز ثابت است و نیازی به تغییر ندارد. با اجرای این endpoint، `LittleFS.format()` انجام می‌شود و همه سناریوها، زمان‌بندی‌ها، تنظیمات WiFi، ساعت ذخیره‌شده، آمار موتور، تنظیمات محافظت و اطلاعات NTP پاک می‌شوند. سپس برد بدون ذخیره دوباره زمان، ری‌استارت می‌شود و به مقادیر پیش‌فرض firmware برمی‌گردد.
