...
برای درخواست‌های تغییردهنده (`POST`) اپ payload را داخل یک envelope می‌فرستد:

```json
{"payload":{...},"checksum":"fnv1a32-hex"}
```

برد قبل از اعمال تنظیمات، payload را دوباره serialize می‌کند و checksum را مقایسه می‌کند. اگر داده در مسیر خراب یا ناقص شده باشد، عملیات انجام نمی‌شود و خطای JSON برمی‌گردد. برای سازگاری، firmware هنوز درخواست‌های قدیمی بدون envelope را هم قبول می‌کند.

### مدیریت عملیات در اپ

اپ فقط یک عملیات فعال دارد. اگر هنگام اجرای عملیات ارتباط با برد قطع باشد، همان پنجره عملیات باز می‌ماند و متنش به‌روزرسانی می‌شود؛ پیام جدید اضافه نمی‌شود. بعد از اتصال موفق، همان عملیات ارسال می‌شود و پس از تأیید برد، پیام موفقیت کوتاه نمایش داده شده و پنجره خودکار بسته می‌شود.
...
- اپ Android به نسخه WebView تغییر کرد: همان CSS/SVG/چیدمان پنل اصلی داخل اپ embed شده است، ESP32 همچنان فقط JSON API می‌دهد، آدرس ESP32 با کلیک روی نشانگر اتصال تغییر می‌کند، پشتیبان‌گیری/بازیابی سناریوها از فایل JSON داخل گوشی انجام می‌شود، اگر اتصال به API برقرار نشود اپ می‌تواند SSID وای‌فای برد را پیشنهاد دهد و با تأیید کاربر برای اتصال تلاش کند، و در تب تنظیمات دکمه «ریست کامل حافظه برد» اضافه شده است.
